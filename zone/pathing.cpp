...
bool PathManager::NoHazards(VERTEX From, VERTEX To)
{
	...
	float NewZ = zone->zonemap->FindBestZ(MidPoint, nullptr, nullptr);
...
bool PathManager::NoHazardsAccurate(VERTEX From, VERTEX To)
{
	...
	do 
	{
		...
		float NewZ = zone->zonemap->FindBestZ(TestPoint, nullptr, nullptr);
		...
		if(zone->watermap)
		{
			...
			if(zone->watermap->InLiquid(TestPoint.x, TestPoint.y, NewZ))
			{
				...
				if(zone->zonemap->LineIntersectsZone(TestPointWater, TestPointWaterDest, 1.0f, &hit, nullptr))
				{
					best_z2 = hit.z;
				}
				if(best_z2 == -999990)
...
void PathManager::MoveNode(Client *c)
{
	...
	if(zone->zonemap)
	{
		...
		Node->bestz = zone->zonemap->FindBestZ(loc, nullptr, nullptr);
	}
	else
	{
		Node->bestz = Node->v.z;
